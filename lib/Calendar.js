    ...
    value: function renderDays(classes) {
      ...
      for (var i = 1; i <= dayCount; i++) {
        ...
        if (disableDaysBeforeToday && Number(dayMoment.diff(_today, "days")) <= -1) {
          days.push({ dayMoment: dayMoment, isPassive: true, isCurrentMonthDay: true });
        } else {
          days.push({ dayMoment: dayMoment, isCurrentMonthDay: true });
        }
      }
      ...
      return days.map(function (data, index) {
        ...
        var isCurrentMonthDay = data.isCurrentMonthDay;
        ...
        return _react2['default'].createElement(_DayCellJs2['default'], _extends({
          ...
          isOutsideMinMax: isOutsideMinMax,
